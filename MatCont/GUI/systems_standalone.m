    ...
    if ~isempty(findstr(matches,'function dydt'))
        ...
        if(gds.sys_type=="DDE")
            ...
            %-_-_-_-_-_-_%
                ...
                if(DDEno>0)
                    for eqNo=(REno+1):dim-1
                        eq=equations(eqNo,:);
                        equation=parseDDE(eq,cor,extractBefore(t,strlength(t)),gds.dim,REno);
...
function eq = parseDDE(eqIn,coords,tempi,dim,REno)
    ...
    for i=1:dim
        eq=strrep(eq,coords(i),strcat("yM(",strcat(sprintf("%d",(i-REno)),")")));
    end
