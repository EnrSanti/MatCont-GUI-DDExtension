...
d1=0;
d2=3;
delayFunctions=[-2*par_b,-par_a];
tau_max=abs(min(delayFunctions));
yM=state((d1*M+1):(d1*M+d2));
VM=state((d1*M+d2+1):end);
TEMP=yM(1)+2;
TEMP1=32+12;
GM = [yM(1)+yM(2)+yM(3)+TEMP;
commonFunctions.interpoly(-par_a,tau_max*UnitNodes,[yM(2);VM(2:d2:end)],BaryWeights)+TEMP1;
commonFunctions.interpoly(-2*par_b,tau_max*UnitNodes,[yM(3);VM(3:d2:end)],BaryWeights)];
