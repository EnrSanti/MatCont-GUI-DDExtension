...
d1=2;
d2=1;
delayFunctions=[-3,-2,-7,-5];tau_max=max(abs(delayFunctions));
ScaledNodes=UnitNodesFun()*tau_max;
ScaledDD=UnitDDFun()/tau_max;
BaryWeights=BaryWeightsFun();
yM=state(1:d2);
...
derState=kron(ScaledDD(2:end,2:end),eye(d1))*UM; %DM*state
TMP=3+yM(1);
out=dot(commonFunctions.interpoly(+thetaCap*(-3-(-2))+-2,ScaledNodes,[0;derState(1:d1:end)],BaryWeights),wCap)*(-2-(-3));
...
derState=kron(ScaledDD(2:end,2:end),eye(d1))*UM; %DM*state
TMP=3+yM(1);
out=dot(commonFunctions.interpoly(+thetaCap*(-7-(-5))+-5,ScaledNodes,[0;derState(2:d1:end)],BaryWeights)+commonFunctions.interpoly(+thetaCap*(-7-(-5))+-5,ScaledNodes,[yM(1);VM(1:d2:end)],BaryWeights)+TMP,wCap)*(-5-(-7));
