function out = Plant_neural_model
out{1} = @init;
out{2} = @fun_eval;
out{3} = [];
out{4} = [];
...
GM = @(x) [
yM(1)-yM(1)^3/3-yM(2)+par_eta*(commonFunctions.interpoly(-par_TAU,tau_max*UnitNodes,[yM(1);VM(1:d2:end)],BaryWeights)-par_v0);
par_r*(yM(1)+par_a-par_b*yM(2))                        ];
KM=[];
...
function userfun1=D0(t,kmrgd,par_a,par_b,par_eta,par_r,par_v0,par_TAU)
	userfun1=(-par_TAU);
