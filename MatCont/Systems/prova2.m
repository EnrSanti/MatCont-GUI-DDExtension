...
d1=3;
d2=0;
delayFunctions=[par_a,2*par_a,par_a,2*par_a,3*par_a,33];
...
KM = derState - kron([dot(commonFunctions.interpoly(-thetaCap*(2.*par_a-(par_a))+par_a,tau_max*UnitNodes,[0;UM(1:d1:end)],BaryWeights).*33,wCap)*(2.*par_a-(par_a))+2;
dot(commonFunctions.interpoly(-thetaCap*(2.*par_a-(par_a))+par_a,tau_max*UnitNodes,[0;UM(1:d1:end)],BaryWeights).*33+commonFunctions.interpoly(-thetaCap*(2.*par_a-(par_a))+par_a,tau_max*UnitNodes,[0;UM(2:d1:end)],BaryWeights),wCap)*(2.*par_a-(par_a))+44;
dot(commonFunctions.interpoly(-thetaCap*(33-(3.*par_a))+3.*par_a,tau_max*UnitNodes,[0;UM(3:d1:end)],BaryWeights),wCap)*(33-(3.*par_a)).^2],ones(M,1));
dydt= [KM;(1/tau_max*dMDM_DDE)*[yM;VM]];
