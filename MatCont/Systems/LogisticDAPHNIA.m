...
GM = par_beta*yM(2)*dot(commonFunctions.interpoly(-thetaCap*(par_amax-(par_a))+par_a,tau_max*UnitNodes,[yM(1);VM(1:d2:end)],BaryWeights),wCap)*(par_amax-(par_a))+dot(commonFunctions.interpoly(-thetaCap*(par_amax-(par_a))+par_a,tau_max*UnitNodes,[yM(1);VM(1:d2:end)],BaryWeights),wCap)*(par_amax-(par_a));
dMDM_DDE=kron(UnitDD(2:end,:),eye(d2));
KM = derState - kron([par_r.*yM(2)                                                                                            ],ones(M,1));
