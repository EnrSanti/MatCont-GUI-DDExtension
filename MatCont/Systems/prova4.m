...
function dydt = fun_eval (t,state,par_TAU)
M=10;
...
d1=0;
d2=2;
delayFunctions=[-par_TAU,-10,-1];
tau_max=abs(min(delayFunctions));
yM=state((d1*M+1):(d1*M+d2));
VM=state((d1*M+d2+1):end);
GM = [-2*yM(1)+dot(3*commonFunctions.interpoly(+thetaCap*(-1-(-10))+-10,tau_max*UnitNodes,[yM(1);VM(1:d2:end)],BaryWeights),wCap)*(-1-(-10));
2*yM(1)+commonFunctions.interpoly(-par_TAU,tau_max*UnitNodes,[yM(2);VM(2:d2:end)],BaryWeights)];
...
% --------------------------------------------------------------------------
function jac = jacobian(t,kmrgd,par_TAU)
% --------------------------------------------------------------------------
function jacp = jacobianp(t,kmrgd,par_TAU)
% --------------------------------------------------------------------------
function hess = hessians(t,kmrgd,par_TAU)
% --------------------------------------------------------------------------
function hessp = hessiansp(t,kmrgd,par_TAU)
%---------------------------------------------------------------------------
function tens3  = der3(t,kmrgd,par_TAU)
%---------------------------------------------------------------------------
function tens4  = der4(t,kmrgd,par_TAU)
%---------------------------------------------------------------------------
function tens5  = der5(t,kmrgd,par_TAU)
