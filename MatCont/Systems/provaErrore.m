...
KM = derState - kron([exp(par_loggamma)./2.*dot(commonFunctions.interpoly(+thetaCap*(-par_tau-(-1))+-1,ScaledNodes,[0;derState(1:d1:end)],BaryWeights).*exp(-commonFunctions.interpoly(+thetaCap*(-par_tau-(-1))+-1,ScaledNodes,[0;derState(1:d1:end)],BaryWeights)),wCap)*(-1-(-par_tau))],ones(M,1));
dydt= [KM];
...
derState=kron(ScaledDD(2:end,2:end),eye(d1))*UM; %DM*state
out=exp(par_loggamma)./2.*dot(commonFunctions.interpoly(+thetaCap*(-par_tau-(-1))+-1,ScaledNodes,[0;derState(1:d1:end)],BaryWeights).*exp(-commonFunctions.interpoly(+thetaCap*(-par_tau-(-1))+-1,ScaledNodes,[0;derState(1:d1:end)],BaryWeights)),wCap)*(-1-(-par_tau));
