function [out,rhs] = LogisticDAPHNIA_N
out{1} = @init;
out{2} = @fun_eval;
out{3} = [];
out{4} = [];
...
GM = par_r*yM(1)*(1-yM(1)/par_k)-par_gamma*yM(1)*dot(commonFunctions.interpoly(-thetaCap*(par_a-(par_amax))+par_amax,ScaledNodes,[0;derState(1:d1:end)],BaryWeights),wCap)*(par_amax-(par_a));
dMDM_DDE=kron(ScaledDD(2:end,:),eye(d2));
KM = derState - kron([par_beta.*yM(1).*dot(commonFunctions.interpoly(-thetaCap*(par_a-(par_amax))+par_amax,ScaledNodes,[0;derState(1:d1:end)],BaryWeights),wCap)*(par_amax-(par_a))],ones(M,1));
...
thetaCap=[1;0.99384;0.97553;0.9455;0.90451;0.85355;0.79389;0.727;0.65451;0.57822;0.5;0.42178;0.34549;0.273;0.20611;0.14645;0.095492;0.054497;0.024472;0.0061558;0];
wCap=[0.0012531;0.011989;0.024377;0.035601;0.046199;0.055512;0.063559;0.069964;0.07471;0.07756;0.078552;0.07756;0.07471;0.069964;0.063559;0.055512;0.046199;0.035601;0.024377;0.011989;0.0012531];

function out = RHSre1(t,state,par_beta,par_r,par_a,par_amax,par_k,par_gamma)
[thetaCap,wCap]=fclencurtVals();
M=10;
d1=1;
d2=1;
delayFunctions=[par_a,par_amax,par_a,par_amax];tau_max=max(abs(delayFunctions));
ScaledNodes=UnitNodesFun()*tau_max;
ScaledDD=UnitDDFun()/tau_max;
BaryWeights=BaryWeightsFun();
yM=state(1:d2);
...
derState=kron(ScaledDD(2:end,2:end),eye(d1))*UM; %DM*state
out=par_beta.*yM(1).*dot(commonFunctions.interpoly(-thetaCap*(par_a-(par_amax))+par_amax,ScaledNodes,[0;derState(1:d1:end)],BaryWeights),wCap)*(par_amax-(par_a));
